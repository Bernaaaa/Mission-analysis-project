function [DeltaV, omf, theta] = changeOrbitalPlaneDeltaT(a, e, i_i, OMi, omi, thi, i_f, OMf, mu)
...
% 
    ...
    cos_alpha = cos(i_i)*cos(i_f) + sin(i_i)*sin(i_f)*cos(abs(dOM));    % Law of cosines for spherical triangles
    alpha = acos(max(-1, min(1, cos_alpha)));   % Numerical safety for acos
    if alpha < 1e-12
        DeltaV = 0; omf = omi; theta = 0; return;
    end

    sin_ui = (sin(abs(dOM)) * sin(i_f)) / sin(alpha); % Law of sines for spherical triangles
    sin_uf = (sin(abs(dOM)) * sin(i_i)) / sin(alpha); % Law of sines for spherical triangles
    ...
    if dI > 0                                   % Same direction of change in RAAN and inclination
    ...
    if tof2 < tof1                            % If the opposite point is farther, check if it offers a better maneuver
        ...
        fprintf('Since it takes less time to arrive at:\ntheta* = theta + pi = %.2f deg\n', rad2deg(theta));
