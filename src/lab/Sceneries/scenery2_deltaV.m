...
x0 = [th_e_grid, th_n_grid, om_t_grid]; 
lb = [0, 0, 0];                            
ub = [2*pi, 2*pi, 2*pi];                   
...
h_t_opt  = cross(r1_opt, r2_opt) / norm(cross(r1_opt, r2_opt));
i_t_opt  = acos(h_t_opt(3));
N_t_vect = cross([0; 0; 1], h_t_opt) / norm(cross([0; 0; 1], h_t_opt));
OM_t_opt = atan2(N_t_vect(2), N_t_vect(1));
...
alpha1 = atan2(r1_p(2), r1_p(1));
alpha2 = atan2(r2_p(2), r2_p(1));

while alpha2 < alpha1, alpha2 = alpha2 + 2*pi; end

th_i_t = alpha1 - om_tn_opt;
th_f_t = alpha2 - om_tn_opt;

r1_n = norm(r1_opt);
r2_n = norm(r2_opt);
e_t  = (r2_n - r1_n) / (r1_n*cos(th_i_t) - r2_n*cos(th_f_t));
p_t  = r1_n * (1 + e_t*cos(th_i_t));
a_t  = p_t / (1 - e_t^2);
...
R_z_om = [cos(om_tn_opt) sin(om_tn_opt) 0; -sin(om_tn_opt) cos(om_tn_opt) 0; 0 0 1];
v1t = RT_ELIO_to_Plane' * (R_z_om' * v1t_pf);
v2t = RT_ELIO_to_Plane' * (R_z_om' * v2t_pf);
